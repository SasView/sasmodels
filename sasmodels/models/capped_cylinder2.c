    ...
    double half_length)
{
    ...
    double integral;
    integrate_kernel(integrand_kernel, lower, upper, lower, m, b, qab_r, &integral);
...
double integrand(double x, double q, double h, double radius_cap, double radius, double half_length, double* F1, double* F2, int n, int i)
{
    ...
    double radius, double radius_cap, double length)
{
    ...
    integrate2(integrand, 0, M_PI_2, q, h, radius_cap, radius, half_length, &total_F1, &total_F2);

    const double form_avg = total_F1;
    const double form_squared_avg = total_F2;
