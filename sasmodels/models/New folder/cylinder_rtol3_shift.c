...
double integrandF1(double x, double A, double B){
    double sin_theta, cos_theta;
    SINCOS(x, sin_theta, cos_theta);
    return sas_sinx_x(A*cos_theta) *  sas_2J1x_x(B*sin_theta) * sin_theta;
}
double integrandF2(double x, double A, double B){
    double sin_theta, cos_theta;
    SINCOS(x, sin_theta, cos_theta);
...
void integrandF1F2(double x, double A, double B, double *F1, double *F2){
    double sin_theta, cos_theta;
    SINCOS(x, sin_theta, cos_theta);
    const double form = sas_sinx_x(A*cos_theta) *  sas_2J1x_x(B*sin_theta);
    *F1 = form * sin_theta;
